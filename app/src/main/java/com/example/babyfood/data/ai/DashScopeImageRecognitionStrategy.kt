...
""".trimIndent()
    ...
    private fun getApiKey(): String {
        // ⚠️ DEPRECATED: 此方法已弃用，不应再使用硬编码的 API Key
        // 请改用 BackendImageRecognitionStrategy 通过后端 API 调用图像识别
        // 后端代理 API: POST /api/v1/images/analyze
        
        Log.w(TAG, "========== DEPRECATED WARNING ==========")
        Log.w(TAG, "DashScopeImageRecognitionStrategy 已弃用!")
        Log.w(TAG, "请使用 BackendImageRecognitionStrategy 通过后端 API 调用图像识别")
        Log.w(TAG, "硬编码 API Key 存在安全风险，将在未来版本中移除")
        Log.w(TAG, "========================================")
        
        throw SecurityException(
            "DashScopeImageRecognitionStrategy 已弃用。" +
            "请使用 BackendImageRecognitionStrategy 通过后端 API 调用图像识别，" +
            "以避免 API Key 泄露风险。"
        )
    }
}
